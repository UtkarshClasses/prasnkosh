...
Widget dateFilterWidget(context) {
        ...
        child: Column(
          children: [
                ...
                children: [
                  ...
                  GestureDetector(
                    onTap: () async {
                      ...
                      List<dynamic> data = await getSuggestionsAPI("date");
                      filterListController
                          .filtersMap['Date']['possible_item'].value = data;
                      EasyLoading.dismiss();
            ...
            Obx(() => selectedItemChips(
                filterListController.filtersMap["Date"]['selected_item'])),
          ],
        ),
      ),
    ),
...
void openFilterDialog(context) async {
  ...
  List<String> selectedItem =
      filterListController.filtersMap['Date']['selected_item'].cast<String>();
  await FilterListDialog.display(
    context,
    height: 500,
    backgroundColor: Colors.yellow,
    listData:
        filterListController.filtersMap['Date']['possible_item'].cast<String>(),
    selectedListData: selectedItem,
    choiceChipLabel: (item) => item,
    validateSelectedItem: (list, val) => list!.contains(val),
    onApplyButtonClick: (list) {
      filterListController.filtersMap["Date"]["selected_item"].value = list;
      Get.back();
    },
